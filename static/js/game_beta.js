...
function PlayerObjBuilder(id, x, y,height,width,imgIdx) {
  ...
  this.ShootFireBall = function (kd) {
     ...
     this.fireBallsActive.push(new fireballObj(this.x+this.width/2,this.y+this.height/2,_direction));
...
var update = function (modifier) {
	...
	if (hero.fireBallDelay <= hero.fireBallDelay_count) {
		hero.fireBallDelay ++;//this is being used as a timer to restrict rate of fire
	};
	/**************MOVEMENT ******************/
	...
	if (87 in keysDown || 65 in keysDown ||83 in keysDown ||68 in keysDown ) {
		if (hero.fireBallDelay > hero.fireBallDelay_count) {
			if (hero.fireBallsActive.length < hero.maxFireBalls) {
				hero.ShootFireBall(keysDown);
				ShotFired = true;}}	}
	
	/************  FIREBALL LOCATION/COLLISION UPDATES  *************/
	/* This is why HERO needs to be in CONNECTED_PLAYER_OBJECTS[0] */
	/******************************************************************/
	for (var fb =0; fb <hero.fireBallsActive.length; fb++) {
			var fireball = hero.fireBallsActive[fb];//USED TO MAKE EASY FOR HUMAN TO READ
			...
			//move the fireball if it's still on the screen
			}else {
			...
			//check if the fireball hit another player remove it	if true
				for (var h=0;h<CONNECTED_PLAYER_OBJECTS.length;h++) {
					if(CollisionCheck(fireball,CONNECTED_PLAYER_OBJECTS[h])){
						hero.fireBallsActive.splice(fb,1);//remove the fireball
						};
					};
				};
	...
	for (var h=0;h<CONNECTED_PLAYER_OBJECTS.length;h++) {
			for (var fb =0; fb <CONNECTED_PLAYER_OBJECTS[h].fireBallsActive.length; fb++)  {
					if (CONNECTED_PLAYER_OBJECTS[h].fireBallsActive[fb].y > canvas.height || CONNECTED_PLAYER_OBJECTS[h].fireBallsActive[fb].y < 0 || CONNECTED_PLAYER_OBJECTS[h].fireBallsActive[fb].x > canvas.width || CONNECTED_PLAYER_OBJECTS[h].fireBallsActive[fb].x < 0) {
					CONNECTED_PLAYER_OBJECTS[h].fireBallsActive.splice(fb,1);//remove the fireball
					}else	{
						CONNECTED_PLAYER_OBJECTS[h].fireBallsActive[fb].x = CONNECTED_PLAYER_OBJECTS[h].fireBallsActive[fb].x +(CONNECTED_PLAYER_OBJECTS[h].fireBallsActive[fb].travelDirection.x *CONNECTED_PLAYER_OBJECTS[h].fireBallsActive[fb].MoveSpeed*modifier);
						CONNECTED_PLAYER_OBJECTS[h].fireBallsActive[fb].y = CONNECTED_PLAYER_OBJECTS[h].fireBallsActive[fb].y +(CONNECTED_PLAYER_OBJECTS[h].fireBallsActive[fb].travelDirection.y*CONNECTED_PLAYER_OBJECTS[h].fireBallsActive[fb].MoveSpeed*modifier);					
						//Check if other players fireball hit you					
						if(CollisionCheck(CONNECTED_PLAYER_OBJECTS[h].fireBallsActive[fb])){
								CONNECTED_PLAYER_OBJECTS[h].fireBallsActive.splice(fb,1);//remove the fireball
								hero.score -= fireball.damage};
						};								
			};			
		};
	...
	if (Object.keys(keysDown).length<1 && hero.direction !== "default"){
		hero.direction = "default"; /* USED SO THIS WON'T TRIGGER EVERY FRAME */
		mySocket.send(JSON.stringify({'mov':{'id':UNIQUE_PLAYER_ID,'x':hero.x,'y':hero.y,'ix':hero.imgIndex,'d':hero.direction}}));
	}
	...
	if (PreviousFramePosition_X !== hero.x || PreviousFramePosition_Y !== hero.y || ShotFired) {
		if (ShotFired) {
			mySocket.send(JSON.stringify({'mov':{'id':UNIQUE_PLAYER_ID,'x':Math.round(hero.x),'y':Math.round(hero.y),'ix':hero.imgIndex,'d':hero.direction,'f':keysDown}}));
		}else {
			mySocket.send(JSON.stringify({'mov':{'id':UNIQUE_PLAYER_ID,'x':Math.round(hero.x),'y':Math.round(hero.y),'ix':hero.imgIndex,'d':hero.direction,'f':false}}));
		}	
	};
};
...
var render = function () {
	...
	//main player
	//if (pRready && pGready) {
	if (PlayerImagesReady && fireBallImgReady) {
		...
		if (CONNECTED_PLAYER_OBJECTS.length>0 ) {
			for (var h=0;h<CONNECTED_PLAYER_OBJECTS.length;h++) {
				...
				for (var fb =0; fb <CONNECTED_PLAYER_OBJECTS[h].fireBallsActive.length; fb++)  {	
				//BOUNDING RECT FOR COLLISION DETECT TESTING
			//	ctx.strokeRect(CONNECTED_PLAYER_OBJECTS[h].fireBallsActive[fb].x,CONNECTED_PLAYER_OBJECTS[h].fireBallsActive[fb].y,CONNECTED_PLAYER_OBJECTS[h].fireBallsActive[fb].sprite.width,CONNECTED_PLAYER_OBJECTS[h].fireBallsActive[fb].sprite.height); 
					CONNECTED_PLAYER_OBJECTS[h].fireBallsActive[fb].sprite.draw(CONNECTED_PLAYER_OBJECTS[h].fireBallsActive[fb].x, CONNECTED_PLAYER_OBJECTS[h].fireBallsActive[fb].y);//shoots a fire ball starting at this xy
				}			
			}		
		};
	}
			...
			*/
			if(JSONdata.id != UNIQUE_PLAYER_ID && JSONdata.id) {
				...
				if (PlayerLookUp.hasOwnProperty(JSONdata.id)) {
						...
						if (JSONdata.f) {
						CONNECTED_PLAYER_OBJECTS[h].ShootFireBall(JSONdata.f);
						};
					}
