...
function PlayerObjBuilder(id, x, y,height,width,imgIdx) {
  ...
  this.maxFireBalls = 5;//max number of onscreen fireballs
...
function fireballObj(x,y,td) {
  ...
  this.MoveSpeed = 20;//movement speed in pixels per frame
  this.x = x;
  this.y = y;
  this.height = 16;
  this.width = 16;
...
var update = function (modifier) {
	...
	/* This is why HERO needs to be in CONNECTED_PLAYER_OBJECTS[] */
	...
	for (var h=0;h<CONNECTED_PLAYER_OBJECTS.length;h++) {
			for (var fb =0; fb <CONNECTED_PLAYER_OBJECTS[h].fireBallsActive.length; fb++)  {
					...
					CONNECTED_PLAYER_OBJECTS[h].fireBallsActive.splice(fb,1);//remove the fireball
						...
						var checkhit = CollisionCheck(CONNECTED_PLAYER_OBJECTS[h].fireBallsActive[fb]);//{console.log("hit")};
...
function CollisionCheck (obj,obj2) {
	...
	if (obj.sprite.testMe) {console.log(obj,obj2)};	
